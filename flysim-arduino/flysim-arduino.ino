...
/* 
  ...
  Last updated by Abel Corver, November 2016
...
#define PC_SERIAL_BUF_LENGTH    1024
...
bool kangarooAvailable(char a=' ') {
  ...
  if (a==' ') { return (kx && kz); } else 
  if (a=='x') { return kx; } else 
  if (a=='z') { return kz; } else {
    // Invalid parameters...
    return false;
  }
}
...
void cmdStatus(char** pArgs, uint8_t numArgs) {
    ...
    StaticJsonBuffer<JSON_MAX_LENGTH> jsonBuffer;
...
void cmdStartKangaroo(char** pArgs, uint8_t numArgs) {
    ...
    if (e != KANGAROO_NO_ERROR) { sendError("Kangaroo Z1 failed: "); Serial.println(e); }
    e = kangarooZ2->home().wait(KANGAROO_COMMAND_TIMEOUT_MS).status().error();
    if (e != KANGAROO_NO_ERROR) { sendError("Kangaroo Z2 failed: "); Serial.println(e); }
    ...
    if (e != KANGAROO_NO_ERROR) { sendError("Kangaroo X failed: "); Serial.println(e); }
...
void cmdTune(char** pArgs, uint8_t numArgs) {
    ...
    if (e != KANGAROO_NO_ERROR) { sendError("KANGAROO_SYS_TUNE_ENTER_MODE failed: "); Serial.println(e); return; }
    ...
    if (e != KANGAROO_NO_ERROR) { sendError("KANGAROO_SYS_TUNE_SET_DISABLED_CHANNELS failed: "); Serial.println(e); return; }
    
    // Begin the tune.
    long goParams[0];
    e = kangarooX->systemCommand(KANGAROO_SYS_TUNE_GO, false, goParams, 0);

    if (e != KANGAROO_NO_ERROR) { sendError("KANGAROO_SYS_TUNE_GO failed: "); Serial.println(e); return; }
...
void cmdZ(char** pArgs, uint8_t numArgs) {
    ...
    if (numArgs >= 1) { z     = String(pArgs[0]).toInt(); }
    if (numArgs == 2) { speed = String(pArgs[1]).toInt(); }
...
void cmdX(char** pArgs, uint8_t numArgs) {
    ...
    if (numArgs >= 1) { x     = String(pArgs[0]).toInt(); }
    if (numArgs == 2) { speed = String(pArgs[1]).toInt(); }
...
void processSerialCommand(char** pArgs, uint8_t numArgs) {
    ...
    if (cmd == "status"     ) { fun = &cmdStatus;        } else 
    if (cmd == "h"          ) { fun = &cmdIdentify;      } else 
    if (cmd == "tune"       ) { fun = &cmdTune;          } else 
    if (cmd == "z"          ) { fun = &cmdZ;             } else
    if (cmd == "x"          ) { fun = &cmdX;             } else 
    if (cmd == "trials"     ) { fun = &cmdTrials;        } else 
    if (cmd == "kangaroo"   ) { fun = &cmdStartKangaroo; } else 
    if (cmd == "abort"      ) { fun = &cmdAbort;         } else 
    if (cmd == "powerdown"  ) { fun = &cmdPowerdown;     } else 
    if (cmd == "neutral"    ) { fun = &cmdNeutral;       } else 
    if (cmd == "speedx"     ) { fun = &cmdSpeedX;        } else  
    if (cmd == "trajectory" ) { fun = &cmdTrajectory;    } else {
...
void loop() {
    ...
    if ( !kx && !kz ) { g_Status = "Unable to connect to either Kangaroo."; } else 
    if ( !kx &&  kz ) { g_Status = "Unable to connect to X Kangaroo."; } else 
    if (  kx && !kz ) { g_Status = "Unable to connect to Z Kangaroo."; } else {
      g_Status = "OK";
    }
